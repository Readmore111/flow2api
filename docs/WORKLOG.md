...
- Verification result:
  ...
  - DNS/TCP breakdown for the same 60-node sample: Windows returned `dns_ok + tcp_ok` for all 60; the VPS had 47 `dns_ok` nodes that still timed out on TCP connect and 13 DNS resolution failures.
