...
## Flow Image Generation Gotchas

- Image-generation progress `48%` corresponds to `generate_image()` entering `submitting_image` after reCAPTCHA is obtained and before/while `flowMedia:batchGenerateImages` is submitted. Failures at this point usually mean the upstream Flow submit request was rejected, not that captcha solving never started.
- `browser` captcha mode has a browser-context submit path: it executes `grecaptcha.enterprise.execute(...)` and submits the Flow request with page-side `fetch(..., credentials: "include")`.
- `personal` captcha mode must also submit image Flow requests inside the resident browser tab. The durable path is `BrowserCaptchaService.submit_flow_request(...)`, which executes reCAPTCHA and the Flow `fetch` in the same browser context. Avoid reintroducing a backend HTTP submit fallback for personal image generation unless there is a specific tested reason.
- Personal-mode protocol cookies live in `tokens.google_cookies`; `tokens.cookie` is legacy session material. Browser cookie binding and cookie write-back should prefer `google_cookies`, with a read-only fallback to the old `cookie` field.
- Nodriver/CDP cookie injection with `browser_context_id` can fail with `Failed to find browser context` when the context was recycled. For startup/binding cookies, retrying `Storage.setCookies` against the global browser cookie jar is a valid fallback and avoids losing the slot unnecessarily.
...
- A subscription working on the user's Windows workstation does not prove it is usable from the VPS. On 2026-07-02, the same `/clash` subscription had the first 60 sampled nodes TCP-reachable from Windows but all failed from the VPS, while the VPS still had normal outbound Internet and direct Google 204 access. Of those 60 VPS failures, 47 resolved DNS but timed out on TCP connect and 13 failed DNS resolution. Treat this as a VPS-to-node reachability/provider-routing problem unless a server-side TCP sample says otherwise.
...
- User/API-client routing changes can affect which Token pool is selected, but they usually fail early with no-token, unbound-pool, cooldown, or concurrency errors. If a request reaches image progress `48%` and fails with upstream `reCAPTCHA evaluation failed`, focus on the browser submit context, browser cookies, proxy/outbound IP, and reCAPTCHA risk score first. A deployment that restarts the container can still expose these issues by discarding a previously warmed browser profile.
